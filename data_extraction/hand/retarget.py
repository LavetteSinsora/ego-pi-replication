...
class HandRetargeter:
    ...
    def retarget(self, hand_pose, timestamps_ns=None, active=None, recalibrate=True):
        ...
        aligned = np.einsum("ij,tfj->tfi", self.R_align, tips)
        scaled = aligned * self.scales[None, :, None]

        # human pad distances for pinch targets (raw, unscaled)
        d_ti = np.linalg.norm(tips[:, 0] - tips[:, 1], axis=1)
        d_tm = np.linalg.norm(tips[:, 0] - tips[:, 2], axis=1)
        ...
        for t in range(T):
            if not valid[t]:
                continue
            f_cmds, f_tips = self._solve_fingers(scaled[t, 1:])
            snap_i, snap_m = d_ti[t] < SNAP_DIST, d_tm[t] < SNAP_DIST
            t_ti = SNAP_TARGET if snap_i else s_ti * d_ti[t]
            t_tm = SNAP_TARGET if snap_m else s_tm * d_tm[t]
            flex, rot, thumb_tip = self._solve_thumb(
                scaled[t, 0], f_tips[0], f_tips[1], t_ti, t_tm, snap_i or snap_m
            )
            cmds[t] = [flex, rot, *f_cmds]
            residual[t, 0] = np.linalg.norm(thumb_tip - scaled[t, 0])
            residual[t, 1:] = np.linalg.norm(f_tips - scaled[t, 1:], axis=1)
            snap_flags[t] = (snap_i, snap_m)
