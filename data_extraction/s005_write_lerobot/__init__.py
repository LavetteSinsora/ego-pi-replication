...
def run_global(cfg, ep_paths):
    ...
    for ep_path in ep_paths:
        ...
        state = np.concatenate(
            [np.concatenate([s003[f"state_eef_{pre[h]}"],
                             s002_02[f"hand_cmds_{pre[h]}"]], axis=1)
        ...
        for start, end, real_end in zip(s004["subep_start"], s004["subep_end"],
            ...
            for t in range(start, end):
                frame = {
                    ...
                    "state": state[t],
                    "arm_qpos": s003["arm_qpos"][t].astype(np.float32),
                    "task": cfg.task_prompt,
                }
                for h in cfg.hands:
                    frame[f"pose.{h}"] = s002_01[f"pose_{pre[h]}"][t]
                    frame[f"hand.{h}"] = s002_02[f"hand_cmds_{pre[h]}"][t]
