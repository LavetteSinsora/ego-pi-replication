...
Usage (always from the repo root, repo venv):
    .venv/bin/python -m data_extraction.run_pipeline                 # all stages, all episodes
    .venv/bin/python -m data_extraction.run_pipeline --through s004 --limit 3
    .venv/bin/python -m data_extraction.run_pipeline --stages s001,s002_02 --force
    .venv/bin/python -m data_extraction.run_pipeline --set control_hz=15 --set repo_id=ego-pi/test
...
STAGE_MODULES = {
    "s001": "data_extraction.s001_uniformalize",
    "s003_placement": "data_extraction.s003_proprioception.placement_stage",
    "b_calib": "data_extraction.s002_action_label.b_alignment",
    "s003_state": "data_extraction.s003_proprioception.state_stage",
    "s002_01": "data_extraction.s002_action_label.eef_label",
    "s002_02": "data_extraction.s002_action_label.hand_label",
    "s004": "data_extraction.s004_filter_split",
    "s005": "data_extraction.s005_write_lerobot",
}

# stages whose inputs include the global B (their signature must change when
# b_calib's output changes, e.g. after adding episodes)
B_DEPENDENT = {"s003_state", "s002_01", "s004"}
